...
-- Marcando compras facturadas
update COMPUMUNDOHIPERMEGARED.Compra
set facturada = 1
where id_compra in (
select c.id_compra from COMPUMUNDOHIPERMEGARED.Compra c
inner join COMPUMUNDOHIPERMEGARED.Item_Factura itf on itf.compra_id = c.id_compra
)
PRINT 'Actualicé compras facturadas'
go
...
return (SELECT top (@cantidad_top) c.id_compra AS compra_ID, c.precio_total AS [Precio total], c.cantidad as [Cantidad],
                ...
                INNER JOIN COMPUMUNDOHIPERMEGARED.Publicacion p ON p.id_publicacion = u.publicacion_id
                INNER JOIN COMPUMUNDOHIPERMEGARED.Espectaculo e ON e.id_espectaculo = p.espectaculo_id
				WHERE empresa_ID = @empresa_id and c.facturada = 0
